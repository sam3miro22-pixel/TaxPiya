...
/**
 * print out url
 * Updated: 15 December 2024 - Ruturaj
 * - URL Sanitization added. List custon action button link issue resolved.
...
{
    // Check if the query string is appended
    if($appendCurrentQueryString){
        $arrQs = request()->query();
        if(!empty($arrQs)){
            // Determine whether to use '?' or '&' based on the presence of '?'
            $separator = strpos($path, '?') === false ? '?' : '&';
            $path = $path . $separator . http_build_query($arrQs);
        }
    }

    // Sanitize the path to remove any extra path segments after '?'
    $urlParts = explode('?', $path, 2); // Split path at the first '?'

    // If there is a query string, we will clean it up
    if (isset($urlParts[1])) {
        // Ensure no '/' is found after the query string starts
        $urlParts[1] = preg_replace('/\/.*$/', '', $urlParts[1]);
    }

    // Reconstruct the URL
    $sanitizedPath = $urlParts[0] . (isset($urlParts[1]) ? '?' . $urlParts[1] : '');

    // Output the final sanitized URL
    echo url($sanitizedPath);
}



/**
 * print out url of specific sized image
 * @method string  print_link
 * @param string  path
 * @return void
 */
 ...
 * Updated: 15 December 2024 by Ruturaj
...
function get_value($fieldname, $default = "")
{
	...
	return old($fieldname, request()->get($fieldname, $default));
}
