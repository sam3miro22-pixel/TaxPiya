...
?>
	...
	public static function page_bread_crumb($page_name, $field_name = null, $field_value = null)
	{
		...
		if(get_value("tag")){
            ...
            $filterField = make_readable($field_name);
        }
		...
		if(get_value("label")){
            ...
            $filterValue = make_readable($field_value);
        }

		if ($filterField) {
		?>
			<nav class="page-header-breadcrumbs mb-2" aria-label="breadcrumb">
				<ol class="breadcrumb m-0 px-3" style="--bs-breadcrumb-divider: '/';">
	...
	public static function img($imgsrc, $width = 50, $height = 50, $imagesize = "small",  $imgclass = "img-fluid")
	{
		if (!empty($imgsrc)) {
			...
			foreach ($arrsrc as $src) {
				$sizeSrc = getImgSizePath($src, $imagesize);
			?>
				<img class="<?php echo $imgclass ?>" width="<?php echo $width ?>" height="<?php echo $height ?>" src="<?php echo url($sizeSrc); ?>" onerror="this.onerror=null;this.src='<?php echo url('images/no-image-available.svg'); ?>';" />
		<?php
			}
		}
	}
	...
	public static function import_form($form_path, $button_text = "")
	{
		$formid = random_str();
		?>
		<button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#import-data-form-<?php echo $formid ?>">
