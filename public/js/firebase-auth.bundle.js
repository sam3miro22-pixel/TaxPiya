...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  // resources/js/firebase-auth.js
  var AUTH_META_KEY = "txp_fb_auth_meta";
  var REDIRECT_FLAG_KEY = "txp_fb_redirect_pending";
  var app = null;
  var auth = null;
  var db = null;
  var redirectBootStarted = false;
  function cfg() {
  ...
  async function syncWithLaravel(idToken, extra = {}) {
    ...
    if (!res.ok || !data.ok) {
      const detail = data.message || (res.status === 419 ? "CSRF token mismatch. Recarga la p\xE1gina e intenta de nuevo." : "No se pudo sincronizar la sesi\xF3n");
  ...
  function onAuthChange(cb) {
    if (!auth) {
      ensureInit().then((ok) => {
        if (ok) onAuthStateChanged(auth, cb);
      });
      return () => {
      };
    }
  ...
  function readPageAuthMeta() {
    const wrap2 = document.querySelector(".txp-firebase-auth");
    const app2 = wrap2?.dataset?.app || null;
    return app2 ? { app: app2 } : {};
  }
...
@firebase/firestore/dist/index.esm2017.js:
   ...
   *)
*/
//# sourceMappingURL=firebase-auth.bundle.js.map
